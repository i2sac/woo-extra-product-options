...
class THWEPOF_Utils_Field {
	...
	private static function get_html_select($name, $field, $section, $value){
		...
		foreach($field->get_property('options') as $option_key => $option_text){
			...
			$option_display = esc_attr__($option_text, 'woo-extra-product-options');
			...
			$input_html .= '<option value="'.esc_attr($option_text).'" '.$selected.'>'.$option_display.'</option>';
		}
		$input_html .= '</select>';
	...
	private static function get_html_radio($name, $field, $section, $value){
		...
		foreach($field->get_property('options') as $option_key => $option_text){
			...
			$option_display = esc_attr($option_text);
			...
			$input_html .= '<label class="radio-wrapper" style="'.$style.'"><input type="radio" id="'.$id.'" name="'. esc_attr($name) .'[]" value="'.esc_attr($option_text).'" '.$checked.'> '.$option_display.'</label>';
			$input_html .= $is_valign ? '<br/>' : '';
			$i++;
		}
